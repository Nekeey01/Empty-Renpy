...
init python:
    i_width, i_height = 700, 270
    i_vspace = 50
    i_frame = Frame("#C1A5A5", 0, 0)
...
screen my_input(**args):
    ...
    frame:
        align (.5, .5)
        background i_frame
        xminimum i_width yminimum i_height
        xmaximum i_width ymaximum i_height
        
        vbox:
            align(.5, .5)
            text "{color=#000000}Говорю же, игра зависла. Ничего не поделаешь, придется ждать.{/color}" align(.5, .5)
            add Null(1, i_vspace)
...
style window:
    ...
    background Image("img/gui/textbox.png", xalign=0.5, yalign=1.0)
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            imagebutton auto "img/menu/pref/game_%s.png" action Start()
            
            imagebutton auto "img/menu/pref/load_%s.png" action ShowMenu("load")
    ###
            imagebutton at  Transform(function = eval('my_rotate')):
                idle ("img/menu/pref/Sett_idle.png")
                hover ("img/menu/pref/Sett_hover.png")
                hovered SetVariable('need_rot', 270)
                unhovered SetVariable('need_rot', 0)
        ...
        ##
        button background None keyboard_focus False xysize (0,0) action Jump("kod") keysym("shift_K_a")
 #       imagebutton auto "img/menu/pref/Sett_%s.png" action ShowMenu("preferences")
...
#            textbutton _("Помощь") action ShowMenu("help")
            ...
            if not main_menu:## Кнопка выхода блокирована в iOS и не нужна на Android.   
                textbutton _("Выход") action Quit(confirm=not main_menu)
            else:
                imagebutton auto "img/menu/pref/exit_%s.png" action Quit(confirm=not main_menu)
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    imagebutton:
                        if logo == True:
                            idle "img/menu/pref/slider_yes.png"
                            action SetVariable("logo", False)
                        else:
                            idle "img/menu/pref/slider_no.png"
                            action SetVariable("logo", True)

                    $ persistent.logo = logo 
